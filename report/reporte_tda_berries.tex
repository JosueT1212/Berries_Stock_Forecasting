...
%% ── Colors ───────────────────────────────────────────────────────────────────
\definecolor{tecblue}{RGB}{0, 82, 165}
...
%% ── Section formatting ───────────────────────────────────────────────────────
\titleformat{\section}
  {\large\bfseries\color{tecblue}}
  {\thesection~|}{0.5em}{}[\vspace{-0.3em}\color{tecblue}\rule{\linewidth}{0.4pt}]
...
\titleformat{\subsubsection}
  {\normalsize\itshape\color{tecblue}}
  {\thesubsubsection}{0.5em}{}

%% ── Header / footer ─────────────────────────────────────────────────────────
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\includegraphics[height=1.6cm]{Tec_logo.png}}
...
\begin{titlepage}
  ...
  {\LARGE\bfseries
    ...
    Índice de Precios al Productor de Berries (FRED)}

  \vspace{2cm}

  \includegraphics[width=0.48\textwidth]{Tec_logo.png}

  \vspace{2cm}

  {\normalsize\itshape
    Aplicación del embedding de Takens y homología persistente de Vietoris-Rips\\[0.2em]
    para el pronóstico del PPI de berries (\texttt{WPUSI01102B}, Jun 2008–Abr 2026)}

  \vspace{0.6cm}

  {\normalsize\bfseries Topología de Datos}\\[0.2em]
  {\normalsize 14/Junio/2026}

  \vspace{0.8cm}

  {\normalsize\bfseries Equipo 1}

  \vspace{0.3cm}

  {\normalsize
    \begin{tabular}{cc}
      \textbf{Nombre completo}          & \textbf{Matrícula} \\[3pt]
      \hline\\[-6pt]
  ...
  {\normalsize Instructor: Dr.\ Hugo García Tecocoatzi}\\[0.2em]
  {\normalsize Tecnológico de Monterrey, Campus Guadalajara}

  \vfill

  {\normalsize Guadalajara, Jalisco, México \quad · \quad Junio 2026}
\end{titlepage}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{serie_temporal_subperiodos.png}
  \caption{Serie temporal del PPI de Berries (WPUSI01102B) con los tres subperiodos históricos
           sombreados. Rojo: crisis financiera 2008-2012. Verde: estabilidad 2013-2019.
           Azul: pandemia e inflación 2020-2026.}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{lccX}
    \toprule
    \textbf{Subperiodo} & \textbf{Pers. máx. H$_1$} & \textbf{Ciclos} & \textbf{Interpretación} \\
    \midrule
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{p{4.2cm}p{1.3cm}X}
    \toprule
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{sesion8_ablacion_forecast.png}
  \caption{Pronóstico vs. valores reales en el conjunto de prueba (estudio de ablación, Sesión 8).
           Línea negra: precio real. Las demás líneas muestran las predicciones de los modelos
           LSTM puro, LSTM+TDA v2 sin exógenas, LSTM+Exógenas sin TDA y LSTM+TDA v2+Exog+Atención.}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{comparacion_mae_barras.png}
  \caption{MAE en el conjunto de prueba para todos los modelos evaluados, ordenados de mayor
           a menor error. Las barras verdes corresponden a modelos que incorporan características
           TDA; las barras azules a modelos sin TDA. La línea roja punteada marca el MAE del
           modelo ganador (LSTM + TDA v2 + Exog + Atención, MAE = 29.11).}
  \label{fig:bar_mae}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{diagramas_persistencia_detalle.png}
  \caption{Diagramas de persistencia de Vietoris-Rips por subperiodo histórico ($d=12$, $\tau=3$).
           Los puntos en H$_0$ representan componentes conexas; los puntos en H$_1$ representan
           ciclos. La distancia a la diagonal indica la persistencia de cada característica topológica.
           El subperiodo 2008-2012 presenta la mayor dispersión en H$_1$ (persistencia máx. = 130.1).}
  \label{fig:persistence_diag}
\end{figure}

\subsubsection{Métricas comparativas (MAE, RMSE, $R^2$)}

La Figura~\ref{fig:metricas3} presenta las tres métricas de evaluación para los modelos
representativos de cada familia, mostrando la ganancia consistente de los modelos con TDA en MAE
y RMSE, y el salto más pronunciado en $R^2$ (de valores negativos para RF sin TDA a 0.57 para
LSTM + TDA v1).

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{metricas_comparacion.png}
  \caption{Comparación de MAE, RMSE y $R^2$ para los modelos más representativos. Barras verdes:
           modelos con TDA. Barras azules: modelos sin TDA. En $R^2$, valores negativos indican
           que el modelo es peor que predecir la media de la serie.}
  \label{fig:metricas3}
\end{figure}

\subsubsection{Calendario y lag — Sesión 10}

La Figura~\ref{fig:sesion10} muestra el impacto de agregar características de calendario y rezago
sobre el LSTM + TDA v1. El Modelo B (calendario + lag) logra seguir mejor los puntos de inflexión
estacionales respecto a la línea base.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{sesion10_forecast.png}
  \caption{Pronóstico en el conjunto de prueba para los tres modelos de Sesión 10: Baseline TDA v1
           (152 características), Modelo A (+calendario, 154) y Modelo B (+calendario+lag, 156).
           El Modelo B alcanza MAE = 30.10, mejora del 5.7\,\% sobre la línea base (31.93).}
  \label{fig:sesion10}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{sesion9_grid_heatmap.png}
  \caption{Mapa de calor del MAE de validación en el grid search de EP Loss
           ($\alpha \in \{2,3,5,8\}$, $\beta \in \{0.1, 0.3, 0.5\}$, 12 combinaciones).
           Colores más fríos (verdes) indican menor error. El mínimo se alcanzó en
           $\alpha=2$, $\beta=0.1$ con MAE$_\mathrm{val} = 23.96$.}
  \label{fig:grid_ep}
\end{figure}
...
El código fuente completo, los notebooks de análisis y los datos de este proyecto están
disponibles públicamente en:
\begin{center}
  \url{https://github.com/JosueT1212/Berries_Stock_Forecasting}
\end{center}

%% ═══════════════════════════════════════════════════════════════════════════
\newpage
\section*{\color{tecblue}Referencias}
...
\begin{description}[leftmargin=1.5em, style=nextline]
  ...
  \item[Tapia Hernández, J. (2026).] \textit{Berries Stock Forecasting — notebooks y datos}.
